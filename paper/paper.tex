...
\begin{quote}
    The exotic particles of interest in high-energy physics (e.g. a Higgs boson) decay far, far before they can interact with the detector.
...
\begin{enumerate}[(a)]
    ...
    See Ref. \cite{baldi-higgs} for a study comparing kinematic-based and high-level classification.% , which outputs classification results.
...
As for FCNs, we describe the key ideas here and relegate some technical details to \myhref{s:appendix-cnn}{Appendix \ref{s:appendix-cnn}}, which may be skipped without loss of continuity.  % Fortunately, CNNs and FCNs have much in common, so we are already well on our way.
...
% Pooling acts only along spatial dimensions, and preserves the channel dimension.
% For any input and pooling kernel dimensions $ I_{j} $, $ K_{j}^{(\text{p})} $ and any kernel stride $ s_{j} $, the output size of a pooling layer along the axis $ j $ is
% \begin{equation}
...
%     % \item Define input images with shape $ C_{\text{in}}, I_{\text{w}}, I_{\text{h}} $.
    % Define input layer.
    % This is a convolutional layer.

%     % Define an arbitrary number of filters to use in the input layer.
    % Each filter has dimensions $ (C_{\text{in}}, K_{\text{w}}, I_{\text{h}}) $.
    % Use $ C_{\text{out}} $ such filters.
    % Perform discrete convolutions of an input image with each filter as described in \myhref{ss:discrete-conv}{Section \ref{ss:discrete-conv}} and shown Figure \ref{fig:conv-multi-channel}.
    % The result is an output with shape $ (C_{\text{out}}, O_{\text{w}}, O_{\text{h}}) $, where $ O_{\text{w}} $ and $ O_{\text{h}} $ as described in \myhref{ss:conv-arithmetic}{Section \ref{ss:conv-arithmetic}}.
...
\end{itemize}
% \textit{Note}: The following two sections contain a lot of information.
% From personal experience, keeping track of the relationships between weights and biases across network layers may be overwhelming to the unfamiliar reader on a first attempt.
% Unfortunately, I know of no way around this if we wish to meaningfully understand a neural network beyond mere hand-waving.
% To make things easier, I have done my best to provide consistent and semantically meaningful notation; also, a more intuitive ``big-picture'' interpretation of a neural network follows in \myhref{ss:fcn-interpretation}{Section \ref{ss:fcn-interpretation}}.
...
We can now fully describe the propagation of an input feature map through a convolutional layer.
Suppose the layer's input feature map has the shape $ (C_{\text{in}}, I_{\text{w}}, I_{\text{h}}) $, and a set of $ C_{\text{out}} $ kernels with spatial dimensions $ (K_{\text{w}}, K_{\text{h}}) $ is used to process the input image.
In this case, the convolutional layer's output has the shape $ (C_{\text{out}}, O_{\text{w}}, O_{\text{h}}) $, where the output dimensions $ O_{j} $ are found from Equation \ref{eq:cnn-O-conv}.
This output can then be used as the input to a subsequent convolutional layer with a different set of kernels, and the convolution process repeats in the next layer, as outlined in \myhref{ss:cnn-architecture}{Section \ref{ss:cnn-architecture}} on CNN architecture.
